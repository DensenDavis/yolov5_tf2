...
class TrainLoop():
    def __init__(self, dataset, model, optimizer):
        ...
        self.stride_losses = [YOLOLoss(cfg.yolo_anchors[i]) for i in cfg.yolo_anchor_masks]
    ...
    def calculate_loss(self, y_true, y_pred):
        ...
        for i in range(cfg.num_strides):
            net_loss += self.stride_losses[i](y_true[i],y_pred[cfg.num_strides-i-1])
        return net_loss
    ...
    def save_predictions(self, input_batch, output_batch, epoch, sample_count):
        ...
        for j in range(cfg.yolo_anchor_masks.shape[0]):
            pred_labels = self.yolo_boxes(output_batch[j], cfg.yolo_anchors[cfg.yolo_anchor_masks[j]])
